...
int main( int argc, char *argv[ ] ) {
	...
	for( int i = 0; i < NUMNODES*NUMNODES; i++ )
	{
		...
		partialSum = Height(iu, iv);
		multiplier = 1;
		
		//change multiplier to 0.25 if corner piece
		if ((iu == 0 && iv == 0) || (iu == (NUMNODES - 1) && iv == 0) ||
			(iu == 0 && iv == (NUMNODES-1)) || (iu == (NUMNODES-1) && iv == (NUMNODES-1))) {
				multiplier = 0.25;
		}
		//change multiplier to 0.5  if edge piece
		else if ((iu != 0 && iv == 0) || (iu != 0 && iv == (NUMNODES-1)) ||
			(iu == 0 && iv != 0) || (iu == (NUMNODES-1) && iv !=0)) {
			multiplier = 0.5;	
		}	
		//otherwise, middle pieces so change multiplier to 1
		else {
			multiplier = 1;
			partialSum *= multiplier;
		}
		volume += double(fullTileArea * partialSum * multiplier);
	}
	time1 = omp_get_wtime();
	megaHeights = (double)(NUMNODES*NUMNODES)/(time1 - time0) / 1000000;
	
	//Print volume
	printf("Volume with %d nodes & %d thread = %8.4lf\n", NUMNODES, NUMT, volume);
	printf("MegaHeights/sec = %8.4lf\n", megaHeights);
}
